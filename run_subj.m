...
% for Excel MSO output
excel_output = array2table(zeros(0,3), 'VariableNames',...
    {'Hair_Thickness', '%MSO', 'Max_E50'});
excel_output_location = [subjects_folder filesep subj filesep outputfolder];
copyfile('MSO.xlsx', [excel_output_location filesep subj '_MSO.xlsx'])
...
tms_opt.fnamecoil = [simnibs_folder filesep coil_models_are_here sep coil_name];
...
for i=hairthicknesses
    ...
    if (~already_run)
        ...
        if (res==0)
            ...
            if (Optimize_ROI_Efield_Magnitude==1)
                ...
                save([subjects_folder sep subj sep target_name  '_HT' sprintf('%.1f',i) sep 'EXX.mat'], 'Evec_value_for_50th_percentile', 'Evec_value_for_75th_percentile', 'Evec_value_for_90th_percentile', 'Evec_value_for_95th_percentile', 'Evec_value_for_99th_percentile', 'Evec_value_for_E100', 'Evec_value_for_E20', 'Evec_value_for_E50');
            end
        end
    end
    ...
    excel_output(height(excel_output)+1, :) = {i round(Scale_Efield_to_Value/Evec_value_for_E50/max_didt*100*(1-Flip_current_direction*2)) Evec_value_for_E50*max_didt};
    disp(['[TAP] --- computations for hairthickness ' num2str(i) ' mm completed ---']);
end
writetable(excel_output, [excel_output_location filesep subj '_MSO.xlsx'], 'Range', 'E1');
HT2MSO_coefs = polyfit(excel_output.Hair_Thickness, Scale_Efield_to_Value*100./excel_output.Max_E50, 2)';
